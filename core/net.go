...
func TcpRead(conn *net.TCPConn) ([]byte, error) {
	...
	bs := make([]byte, MESSAGE_PREFIX, MESSAGE_PREFIX)
	bbs := make([]byte, 1024 * 1000 * 5)
	conn.Read(bs)
	readLength := int32(MESSAGE_PREFIX)

	datalength := BytesToInt(bs[1:])

	for readLength < datalength {
		n, err := conn.Read(bbs[readLength - MESSAGE_PREFIX:])
		if err != nil {
			return nil, err
		}
		readLength += int32(n)
	...
	return append(bs, bbs[0:readLength - MESSAGE_PREFIX]...), nil
}
